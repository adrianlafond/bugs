...
export class Bug010 extends BaseDemo {
  ...
  private renderSegment (segment: SegmentData, activeSide: BugSide, index: number): void {
    ...
    if (index === 0) {
      ...
      gfx.moveTo(-segmentRadius[index], -segmentRadius[index])
      gfx.lineTo(segmentRadius[index], segmentRadius[index])
      gfx.moveTo(segmentRadius[index], -segmentRadius[index])
      gfx.lineTo(-segmentRadius[index], segmentRadius[index])
